    ...
    instantsearch.widgets.ratingMenu({
        ...
        <a
          ...
          <div>
			  ...
			  <span class="${data.cssClasses.label} text-primary">&amp; Выбрать</span>
		</div>
...
const renderToggleRefinement = (renderOptions, isFirstRender) => {
	...
	document.querySelector('#toggle-refinement').innerHTML = `
		<div class="custom-control custom-checkbox">
			...
			<label class="custom-control-label" for="free_Ship">Выгодное предложение</label>
		</div>
  `;
...
search.addWidget(
    instantsearch.widgets.sortBy({
        ...
        items: [
            { label: 'Рекомендуемые', value: 'instant_search' },
            { label: 'Цена (Низкая)', value: 'instant_search_price_asc' },
            { label: 'Цена (Высокая)', value: 'instant_search_price_desc' },
        ],
    })
);
...
search.addWidget(
    instantsearch.widgets.numericMenu({
        ...
        items: [
            { label: 'Все' },
            { label: 'До 500 рублей ', end: 500 },
            { label: 'До 1000 рублей ', start: 500, end: 1000 },
            { label: 'От 1000 рублей ', start: 1000 },
        ],
    })
);
